...
contract SimpleBank {
    ...
    modifier onlyOwner(){
      require(enrolled[msg.sender],"not enroll");
      _;
    }

    modifier enoughBalance(uint withdrawAmount) {
    ...
    function withdraw(uint withdrawAmount) public onlyOwner enoughBalance(withdrawAmount) returns (uint) {
